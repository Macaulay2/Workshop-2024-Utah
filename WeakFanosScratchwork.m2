...
-- Generator of all Fano toric 4-folds with 3 p.c. that are projective bundles over 3-folds
...
-- Generator of all weak Fano toric 4-folds with 3 p.c. that are projective bundles over surfaces

newnewProjectiveBundleConstructor = (X, l1, l2) -> (
    assert(length l1 == 2);
    assert(length l2 == 2);
    zeroD = toricDivisor({0,0,0,0}, X);
    D1 = toricDivisor(join(l1, {0,0}), X);
    D2 = toricDivisor(join(l2, {0,0}), X);
    projectivizationOfBundle({zeroD, D1, D2})
);

counter = 0;
for b from 0 to #bases2D-1 do (
    for c1 from 0 to 4 do (
	for c2 from -4 to 4 do (
	    for c3 from -4 to 4 do (
		for c4 from -4 to 4 do (
		    if (not ((c1 == 0 and c2 < 0))) and (c1+c2 <= c3+c4) and (not (c1+c2 == c3+c4 and c1 > c3)) then (
			if counter < 100 then (
			    base2D = bases2D#b;
			    X = newnewProjectiveBundleConstructor(base2D, {c1, c2}, {c3, c4});
...
-- seems like all weak Fanos generated by above, involve coefficients <= 2? except for one lone exception: (0, 0), (3, 2)54. base: 1
-- when we expand to 4, we get (0,0), (4,2)
-- I would venture these are the only exceptions (shrug)
