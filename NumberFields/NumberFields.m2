...
numberField(Ring) := opts -> R1 -> (
    ...
    if R1===QQ or (coefficientRing R1 === QQ and gens R1 === {}) then (   
        myVar := getSymbol "ww";
        if not opts.Variable === null then myVar = opts.Variable;
        amb := QQ[myVar];
        tempNumField = toField (amb/ideal(gens amb));  
        myMap := map(tempNumField,R1);     
        tempMap := map(coefficientRing tempNumField, R1)   ;
        if not (tempNumField#?cache) then tempNumField#cache = new CacheTable from {};
        tempNumField#cache#NumberField = true;
        tempNumField#cache#pushFwd = pushFwd(tempMap);
        tempNumField#cache#degree = 1;
        tempNumField#cache#internalNFMaps = (myMap, map(R1, tempNumField));        
        return tempNumField;
    );
    local outputRing;
    local outputPsi;
...
--    if opts.Verbose or (debugLevel > 1) then print ("numberField:  remakeField called, " | toString(intermediateRing));
    ...
    while (i < genCt) do (
        ...
        myStr = myStr | " (" | toString ((genMinPolys#i)#0) | ")";
        i = i+1;
...
splittingField = method(Options => {cache => true, Variable=>null, Verbose=>false, UsePari => defaultPariStrat})
...
splittingFieldNonPari = method(Options => {cache => true, Variable=>null, Verbose=>false, UsePari => defaultPariStrat})
    ...
    if (isNumberField coefficientRing R1) then (
        ...
        while not finished do (
            ...
            if (#factorList > 0) then (
                ...
                finished = false;
                tempTargetRing = R1/ideal(currentEntry);
                inclusionMap = map(tempTargetRing, K1); --include the field into the ring                
                --first we make the larger field extension
                (K1new, mapToK1new, mapFromK1new)  = extraFlattenRing(tempTargetRing);
                (K1new2, mapToK1new2, mapFromK1new2)  = internalSimpleExtension(K1new, Variable=>var);
                fieldExtensionMap =mapToK1new2*mapToK1new*inclusionMap; --this is the field Extension
                --now we make the map from the old ring to the new one
                K1new2asField = toField K1new2;
                R1new = K1new2asField[tempvar];
                inclusionMap = map(R1new, K1new2);
                newRingMap = map(R1new, R1, {(gens R1new)#0, (inclusionMap*fieldExtensionMap)((gens K1)#0)});
                linTerm = ((gens R1new)#0) - (inclusionMap*mapToK1new2*mapToK1new*(map(tempTargetRing, R1)))((gens R1)#0);
                --now we simplify the factorList
                factorList =  {(newRingMap currentEntry)//linTerm} | apply(drop(factorList, 1), tt->newRingMap tt);                
                myFactors = flatten apply(factorList, tt -> (zz := factor tt; apply(#zz, ss -> zz#ss#0 )));
                factorList = select(myFactors, tt->degree(first gens R1new, tt) > 1);
                R1 = R1new;
                K1 = K1new2;
            );
        );      
        finalField := numberField K1new2;  
        return (finalField, (sourceRingOfNumberField finalField)*fieldExtensionMap);
    )
    else (
        ...
        while not finished do (
            ...
            if (#idealList > 0) then (
                ...
                if (opts.Verbose) then print idealList;
                if opts.Verbose then print ("doing a saturate " | toString(phi1 curIdeal) | " against " | toString(linTerm)) ;
                ...
                if (#idealList == 0) and (#currentEntry == 1) and (degree(Svar, phi1(currentEntry#0)) <= 2) then (
                    ...
                    idealList = (decompose (newIdeal, Strategy=>"Legacy")) | flatten apply(idealList, j->decompose(j, Strategy=>"Legacy"));
                    idealList = select(idealList, z->not isLinear z);        --let's only keep the good ones.
                    R1 = S1;
                    variableIndex += 1;
                )
...
isLinear(Ideal) := opts -> (J1) -> (
    ...
    myVar := (gens ring J1)#0;
    all(apply(idealGens, j -> degree(myVar, j)), i->i<=1)
)
...
internalSimpleExtension(Ring) := opts -> nf ->(
    ...
    if (coefficientRing nf === QQ) and (gens nf ==={}) then return (nf, id_nf, id_nf);
    ...
    if (opts.cache) and (nf#cache#?internalSimpleExtension) then (( if (opts.Verbose) then print "internalSimpleExtension: found cached valued, returning";); return nf#cache#internalSimpleExtension;);
